...
//! record before this boundary is crossed. The serialized invocation contains
//! no host path, credential, or ambient conversation state. A provider must use
//! the stable invocation ID as an idempotency key so executor replay cannot
...
//! cleared environment, so it does not inherit environment-provided credentials
//! or endpoints from Kaname. The transport is not a filesystem, network, or
//! current-directory sandbox; Candidate wiring must add and qualify that
//! isolation before using an untrusted provider executable:
//!
