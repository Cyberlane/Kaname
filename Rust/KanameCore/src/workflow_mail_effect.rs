...
impl WorkflowMailEffectClass {
    ...
    /// The egress class the approval scope pins. `send` delivers content to a
    /// recipient, `draft` writes content to the bound remote mailbox without
    /// recipient delivery, and the remaining kinds mutate existing mailbox
    /// state.
    ...
    const fn consequence(self) -> &'static str {
        match self {
            Self::Send => "The approved message will be delivered to its recipient",
