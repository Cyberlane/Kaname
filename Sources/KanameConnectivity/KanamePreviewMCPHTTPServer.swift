...
extension CodexMCPIsolation {
    /// Writes an ephemeral Codex config that enables only the Kaname-managed
    /// MCP servers: the curated preview bridge (when granted) and the Kaname
    /// Bridge for plan, findings, and knowledge.
    static func writeKanameMCPConfig(
        home: URL,
        preview: KanamePreviewMCPHTTPServer.Binding?,
        bridge: KanameBridgeMCPServer.Binding?
    ) throws {
        var sections = ["# Generated by Kaname. Do not copy into ~/.codex."]
        if let preview {
            sections.append("""
        ...
        if let bridge {
            sections.append("""
            [mcp_servers.\(KanameBridgeMCPServer.serverName)]
            url = "\(bridge.url.absoluteString)"
            bearer_token_env_key = "KANAME_BRIDGE_MCP_TOKEN"
            """)
        }
    ...
    static func curatedPreviewEnvironment(
        ...
        if let bridge {
            environment["KANAME_BRIDGE_MCP_TOKEN"] = bridge.bearerToken
        }
        return environment
    }
}
