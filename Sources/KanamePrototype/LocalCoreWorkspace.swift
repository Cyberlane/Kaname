...
private struct LocalCoreRunCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(run.descriptor.title).font(.headline).foregroundStyle(Nord.snowStorm0)
            Text("Task: \(run.report.taskState) · Health: \(run.report.health)")
                .font(.subheadline).foregroundStyle(Nord.snowStorm0.opacity(0.78))
            Text("\(run.report.eventCount) journal events · \(run.report.effectCount) fake effects")
                .font(.caption).foregroundStyle(Nord.snowStorm0.opacity(0.62))
        }
        .padding(16)
