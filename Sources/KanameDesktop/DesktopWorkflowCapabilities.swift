...
public enum DesktopWorkflowBuiltinCapabilities {
    ...
    private static func metadata(for identifier: String) -> (name: String, summary: String) {
        ...
        case "kaname.email.read": ("Mail reader", "Reads account-scoped provider deltas, conversations, resources, and policy-selected attachments.")
        case "kaname.email.draft": ("Mail draft writer", "Creates and reconciles an exact provider-backed mail draft.")
        case "kaname.email.send": ("Mail sender", "Sends and reconciles an exactly approved provider-backed message.")
