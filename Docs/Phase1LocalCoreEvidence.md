...
| Native restart/replay | pass | F-01 was run through the app, the local service was restarted, and the app replayed the persisted F-01 journal as accepted/none/ready with nine events and one fake effect. A checksummed, bounded fixture-report cache now returns the recorded outcome rather than re-executing a completed fixture; the persisted-core test verifies that behavior for F-01 through F-14. |
