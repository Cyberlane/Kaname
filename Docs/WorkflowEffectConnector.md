...
Only `send` moves content off the device and only `send` cannot be undone from
the same mailbox afterwards. A draft is written into the account but never
delivered, so it earns its own egress class rather than sharing either
neighbour's.
