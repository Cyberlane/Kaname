...
The installed app is `~/Applications/Kaname.app`. Installation also bootstraps the user-scoped `com.cyberlane.kaname.desktop.localcore.service` LaunchAgent. The build and installer never call `security`, access the login Keychain, or enumerate physical devices.
