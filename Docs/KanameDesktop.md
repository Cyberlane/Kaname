...
The installed app is `~/Applications/Kaname.app`. The current dogfood bundle is version `0.8.0` (build `17`). Projects open into a deliberate context overview with editable purpose, workspace, instructions, knowledge, skills, and conversation defaults; the project-tile compose shortcut remains a separate one-click action. The standard thread composer now starts or continues a durable, read-only Codex conversation, streams provider text and activity into the same timeline, queues follow-ups, supports interrupt/retry and questions, retains bounded raw event evidence, and asks Codex for a title after the first successful turn. Provider history lives in Kaname's private isolated Codex home so restart can resume without importing the user's Codex configuration or MCP/app servers. Installed and development builds share a user-scoped process lock, so a second launch activates the existing Kaname UI and exits before creating another window. Settings uses an in-window modal whose dimmed backdrop, Escape command, mouse Back command, and Return to Kaname button all dismiss it; clicks inside the panel remain inside Settings. Installation also bootstraps the user-scoped `com.cyberlane.kaname.desktop.localcore.service` LaunchAgent. Verification posts a button-3 event only into Kaname's own AppKit event queue and now fails unless a real Calendar-to-Home navigation handles it, without Accessibility permission or global input injection. Snapshot qualification disables live provider and integration monitoring and never enumerates accounts, requests Calendar access, or enumerates physical devices. Ad-hoc builds do not access a signing key; a privately configured stable build asks `codesign` to use its selected local Keychain identity.
